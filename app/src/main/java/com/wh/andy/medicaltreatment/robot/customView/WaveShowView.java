package com.wh.andy.medicaltreatment.robot.customView;
...
public class WaveShowView extends View {
    ...
    private ArrayList<Float> refreshList = new ArrayList();//后加的数据点
    private int row;//背景网格的行数和列数

    //心电
    private float MAX_VALUE = 40;
    private float WAVE_LINE_STROKE_WIDTH = 1.5f;
    private int mWaveLineColor = Color.parseColor("#FF3636");//波形颜色
    private float nowX, nowY;//目前的xy坐标

    //网格
    private final float GRID_SMALL_WIDTH = 1f;//每一个网格的宽度和高度,包括线
    private final int GRID_BIG_WIDTH = 125;//每一个大网格的宽度和高度,包括线
    private final int GRID_IN_SMALL_WIDTH = 25;//每一个大网格的宽度和高度,包括线
    private int xSmallNum, ySmallNum, xBigNum, yBigNum, xBigNumSmall, yBigNumSmall;//小网格的横格，竖格，大网格的横格，竖格数量
    private final float GRID_LINE_WIDTH = 0.5f;//网格的线的宽度
    private int mWaveSmallLineColor = Color.BLACK;//小网格颜色
    private int mWaveBigLineColor = Color.parseColor("#65FFFFFF");//小网格颜色


    private int mWaveSmallBigLineColor = Color.parseColor("#65FFFFFF");//分割网格颜色
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        xBigNumSmall = (int) (mHeight / GRID_IN_SMALL_WIDTH);//横线个数
        yBigNumSmall = (int) (mWidth / GRID_IN_SMALL_WIDTH);//竖线个数
    }
    ...
    private void drawWaveLine(Canvas canvas) {
        ...
        for (int i = 0; i < refreshList.size(); i++) {
            ...
            if (refreshList != null && refreshList.size() > 0 && refreshList.get(i) != null) {
                ...
                nowY = getMeasuredHeight() - dataValue * (mHeight / (MAX_VALUE));
  ...
  /*  private void drawWaveLine(Canvas canvas) {
        ...
        for (int i = 0; i < refreshList.size(); i++) {
            nowX = i * GRID_SMALL_WIDTH;
         *//*   if(!TextUtils.isEmpty(refreshList.get(i))){
...
*//*
    ...
    private void drawGrid(Canvas canvas) {
        ...
        //画小小网格
        mLinePaint.setColor(mWaveSmallBigLineColor);
        //画横线
        for (int i = 0; i < xBigNumSmall + 1; i++) {
            canvas.drawLine(0, i * GRID_IN_SMALL_WIDTH,
                    mWidth, i * GRID_IN_SMALL_WIDTH, mLinePaint);
        }
        //画竖线
        for (int i = 0; i < yBigNumSmall + 1; i++) {
            canvas.drawLine(i * GRID_IN_SMALL_WIDTH, 0,
                    i * GRID_IN_SMALL_WIDTH, mHeight, mLinePaint);
        }
    }
